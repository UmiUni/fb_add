...
def exitAndAlert():
    print("friend request queue reaches 1000!!")
    sys.exit()
...
friendIdQFilePath = "friendIdQFile.pkl"
friendIdQFile = open(friendIdQFilePath, 'ab+')
friendIdQ = None
friendRequestNumPath = "friendRequestNum.pkl"
friendRequestNumFile = open(friendIdQFilePath, 'ab+')
friendRequestNum = None
# if os.path.isfile(friendIdQFilePath):
try:
    friendIdQ = pickle.load(friendIdQFile)
    friendRequestNum = pickle.load(friendRequestNumFile)
except EOFError:
    friendIdQ = deque()
    friendIdQ.append('superchaoran')  # 'https://www.facebook.com/superchaoran/friends'
    friendRequestNum = 247
# else:
#     friendIdQ = deque()
#     friendIdQ.append('superchaoran')  # 'https://www.facebook.com/superchaoran/friends'
    ...
    while (noFriendFoundCtr < 5):
        ...
        for target_rect in target_rects:
            ...
            if (isChinese):
                ...
                pickle.dump(friendIdQ, friendIdQFile)
                friendRequestNum += 1
                pickle.dump(friendRequestNum, friendRequestNumFile)
                if (friendRequestNum == 1000):
                    exitAndAlert()
                gui_bot.random_move_to(button_x, button_y)
